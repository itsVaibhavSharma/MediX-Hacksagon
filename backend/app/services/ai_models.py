...
class SkinDiseaseClassifier(nn.Module):
    """Updated to use ResNet50-based EfficientNet as per your training"""
    def __init__(self, num_classes=22, dropout_rate=0.5):
        super(SkinDiseaseClassifier, self).__init__()
        # Use ResNet50 as backbone (same as your training code)
        self.backbone = models.resnet50(weights=None)
        
        # Get number of features from the backbone
        num_features = self.backbone.fc.in_features
        
        # Replace final classifier with the same architecture as your training
        self.backbone.fc = nn.Sequential(
            nn.Dropout(dropout_rate),
            nn.Linear(num_features, 1024),
            nn.BatchNorm1d(1024),
            nn.ReLU(inplace=True),
            nn.Dropout(dropout_rate * 0.5),  # 0.25 if dropout_rate=0.5
            nn.Linear(1024, 512),
            nn.BatchNorm1d(512),
            nn.ReLU(inplace=True),
            nn.Dropout(dropout_rate * 0.3),  # 0.15 if dropout_rate=0.5
            nn.Linear(512, num_classes)
        )
